...
A successful run prints the security group list in JSON format.
...
| `User has no permission` | IAM policy missing `vpc:securityGroups:list` — see `references/vpc-policies.md` |
| `Missing required parameter --cli-region` | Add `--cli-region=<region>` to the command |
| Empty result | The region has no security groups, or the project has no VPC resources — try another region |
