...
check "Critical" "references/vpc-policies.md exists" "$([ -f "$SKILL_PATH/references/vpc-policies.md" ] && echo PASS || echo FAIL)"
check "Critical" "No credential hardcoding" "$(grep -qiE '(AK=|SK=|access.key=|secret.key=|pa''ss''word=)' "$SKILL_MD" && echo FAIL || echo PASS)"
...
check "Medium" "Service name VPC uppercase" "$(grep -q 'hcloud VPC' "$SKILL_MD" && echo PASS || echo FAIL)"
check "Medium" "Includes ListSecurityGroups command" "$(grep -q 'hcloud VPC ListSecurityGroups' "$SKILL_MD" && echo PASS || echo FAIL)"
