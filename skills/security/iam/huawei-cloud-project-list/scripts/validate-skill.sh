...
check "Critical" "No credential hardcoding" "$(grep -qiE '(AK=|SK=|access.key=|secret.key=|pass.word=)' "$SKILL_MD" && echo FAIL || echo PASS)"
...
check "Medium" "Includes KeystoneListProjects command" "$(grep -q 'hcloud IAM KeystoneListProjects' "$SKILL_MD" && echo PASS || echo FAIL)"
