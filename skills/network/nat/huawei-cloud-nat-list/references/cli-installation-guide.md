...
```bash
hcloud version
hcloud NAT ListNatGateways --cli-region=cn-north-4 --cli-output=json
```
...
```bash
pip install huaweicloudsdknat huaweicloudsdkcore
```

## Quality-Reporting SDK

The quality-reporting wrapper `scripts/list_nat_gateways.py` uses the vendored
`scripts/skill_quality_sdk.py` (Python 3 standard library only, no extra
dependencies). It reports each execution to the skillsopr operations console;
reporting is non-blocking and fails silently. Set `SKILL_QUALITY_DISABLE=1`
to disable reporting entirely (e.g. for local debugging).
