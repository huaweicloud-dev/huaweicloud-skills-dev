...
check "Critical" "Mandatory quality SDK vendored" "$([ -f "$SKILL_PATH/scripts/skill_quality_sdk.py" ] && grep -q 'quality_report\|quality_context' "$SKILL_PATH/scripts/list_nat_gateways.py" && echo PASS || echo FAIL)"
...
check "Medium" "Service name uppercase" "$(grep -q 'hcloud NAT' "$SKILL_MD" && echo PASS || echo FAIL)"
