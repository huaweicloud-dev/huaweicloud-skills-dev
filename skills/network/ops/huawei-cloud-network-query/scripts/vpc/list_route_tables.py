...
from huaweicloudsdkvpc.v2.model import ListRouteTablesRequest
...
parser = argparse.ArgumentParser(description="查询路由表列表")
...
parser.add_argument("--id", type=str, help="路由表 ID 过滤")
parser.add_argument("--subnet_id", type=str, help="子网 ID 过滤，可通过 list_subnets.py 获取")
...
def render(items, total_count=None, has_more=False, next_marker=None):
    """
    渲染路由表列表
    :param items: 路由表列表（最多 PAGE_SIZE 条）
    ...
    output = f"id\tvpc_id\tname\tdefault\tdescription\n"
    for item in items:
        id = getattr(item, 'id', '')
    ...
    if total_count is not None:
        output += f"\n共 {total_count} 条路由表，当前返回 {showing_count} 条"
        if has_more and next_marker:
            output += f"\n可使用 --marker={next_marker} 继续查询下一页，或使用 --id / --vpc_id / --subnet_id 等参数缩小查询范围"
        elif total_count > showing_count:
            output += f"\n可使用 --id / --vpc_id / --subnet_id 等参数缩小查询范围"
    elif has_more:
...
try:
    ...
    print(f"vpc.list_route_tables 查询失败: {e}")
    exit(1)
