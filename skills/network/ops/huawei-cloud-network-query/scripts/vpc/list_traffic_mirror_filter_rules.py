...
from huaweicloudsdkvpc.v3.model import ListTrafficMirrorFilterRulesRequest
...
# 参数
parser = argparse.ArgumentParser(description="查询流量镜像过滤规则列表")
parser.add_argument("--project_id", type=str, default=None, help="项目 ID，不传时自动通过 IAM API 获取")
...
parser.add_argument("--id", type=str, help="规则 ID 过滤")
parser.add_argument("--direction", type=str, help="方向过滤(ingress/egress)")
parser.add_argument("--protocol", type=str, help="协议过滤(tcp/udp/icmp等)")
parser.add_argument("--source_cidr_block", type=str, help="源网段过滤")
parser.add_argument("--destination_cidr_block", type=str, help="目的网段过滤")
parser.add_argument("--action", type=str, help="策略过滤(accept/reject)")
parser.add_argument("--priority", type=str, help="优先级过滤")
...
def render(items, total_count=None, has_more=False, next_marker=None):
    """
    渲染流量镜像过滤规则列表
    :param items: 规则列表（最多 PAGE_SIZE 条）
    ...
    output = f"id\ttraffic_mirror_filter_id\tdirection\tprotocol\tsource_cidr_block\tdestination_cidr_block\taction\tpriority\n"
    for item in items:
        id = getattr(item, 'id', '')
    ...
    if total_count is not None:
        output += f"\n共 {total_count} 条流量镜像过滤规则，当前返回 {showing_count} 条"
...
try:
    ...
    if args.source_cidr_block:
        request.source_cidr_block = args.source_cidr_block
    if args.destination_cidr_block:
        request.destination_cidr_block = args.destination_cidr_block
    if args.action:
    ...
    if has_custom_filter:
        ...
        while True:
            ...
            items = response.traffic_mirror_filter_rules
            if not items:
                break
            # 检测重复数据
        ...
        if not all_items:
            print(f"没有找到流量镜像过滤规则 (区域: {Region})")
            exit(0)
    ...
    print(f"vpc.list_traffic_mirror_filter_rules 查询失败: {e}")
    exit(1)
