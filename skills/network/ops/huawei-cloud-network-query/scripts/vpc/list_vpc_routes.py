...
from huaweicloudsdkvpc.v2.model import ListVpcRoutesRequest
...
parser = argparse.ArgumentParser(description="查询VPC路由列表")
parser.add_argument("--project_id", type=str, default=None, help="项目 ID，不传时自动通过 IAM API 获取")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--marker", type=str, help="分页标记，从上次查询结果的 next_marker 获取")
parser.add_argument("--id", type=str, help="路由 ID 过滤")
parser.add_argument("--type", type=str, help="路由类型过滤(peering)")
parser.add_argument("--vpc_id", type=str, help="VPC ID 过滤，可通过 list_vpcs.py 获取")
parser.add_argument("--destination", type=str, help="目的地址 CIDR 过滤")
...
def render(items, total_count=None, has_more=False, next_marker=None):
    """
    渲染VPC路由列表
    :param items: 路由列表（最多 PAGE_SIZE 条）
    ...
    output = f"id\ttype\tdestination\tvpc_id\tnexthop\ttenant_id\n"
    for item in items:
        id = getattr(item, 'id', '')
        ...
        output += f"{id}\t{route_type}\t{destination}\t{vpc_id}\t{nexthop}\t{tenant_id}\n"

    # 汇总信息
    showing_count = len(items)

    if total_count is not None:
        output += f"\n共 {total_count} 条VPC路由，当前返回 {showing_count} 条"
        if has_more and next_marker:
            output += f"\n可使用 --marker={next_marker} 继续查询下一页，或使用 --id / --type / --vpc_id 等参数缩小查询范围"
        elif total_count > showing_count:
            output += f"\n可使用 --id / --type / --vpc_id 等参数缩小查询范围"
    elif has_more:
...
try:
    ...
    # 只做一次查询
    response = client.list_vpc_routes(request)
    items = response.routes

    if not items:
        print(f"没有找到VPC路由 (区域: {Region})")
        exit(0)
    ...
    print(f"vpc.list_vpc_routes 查询失败: {e}")
    exit(1)
