...
from huaweicloudsdkdns.v2.model import ShowWebsiteRecordSetRequest
from huaweicloudsdkdns.v2.region.dns_region import DnsRegion

AK, SK, Region, SecurityToken = load_credentials()
Offset = 0

parser = argparse.ArgumentParser(description="查询公网域名的网站域名")
...
def render(recordsets):
    ...
    if Offset >= total:
        print(f"查询结果为空\n\n网站记录集列表共 {total} 个, 序号从 0 开始, offset必须 < {total}")
        exit(0)
    ...
    if total > 50 or Offset > 0:
        output += f"\n网站记录集列表共 {total} 个, 序号从 0 开始，当前显示第 {Offset}~{end} 个\n"
        if end + 1 < total:
...
try:
    ...
    if not recordsets:
        print(f"没有找到网站域名记录集 (zone_id: {args.zone_id})")
        exit(0)

    render(recordsets)
except Exception as e:
    print(f"dns.show_website_record_set 查询失败: {e}")
    exit(1)
