...
from huaweicloudsdkdns.v2.model import ListTagsRequest
from huaweicloudsdkdns.v2.region.dns_region import DnsRegion

AK, SK, Region, SecurityToken = load_credentials()

parser = argparse.ArgumentParser(description="查询指定实例类型的所有标签集合")
...
try:
    ...
    if not tags:
        print(f"没有找到标签 (resource_type: {args.resource_type})")
        exit(0)

    render(tags)
except Exception as e:
    print(f"dns.list_tags 查询失败: {e}")
    exit(1)
