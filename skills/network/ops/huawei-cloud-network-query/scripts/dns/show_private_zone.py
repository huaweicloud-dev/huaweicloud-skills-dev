import argparse
import sys
import os

sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

from config import load_credentials, build_http_config, resolve_project_id
from huaweicloudsdkcore.auth.credentials import BasicCredentials
from huaweicloudsdkdns.v2 import DnsClient
from huaweicloudsdkdns.v2.model import ShowPrivateZoneRequest
from huaweicloudsdkdns.v2.region.dns_region import DnsRegion

AK, SK, Region, SecurityToken = load_credentials()

parser = argparse.ArgumentParser(description="查询内网域名")
parser.add_argument("--project_id", type=str, default=None, help="项目 ID，不传时自动通过 IAM API 获取")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--zone_id", type=str, required=True, help="内网域名的ID")
args = parser.parse_args()

if args.region is not None:
    Region = args.region

args.project_id = resolve_project_id(Region, args.project_id)


def render(resp):
    output = ""
    zid = getattr(resp, 'id', '')
    name = getattr(resp, 'name', '')
    description = getattr(resp, 'description', '')
    email = getattr(resp, 'email', '')
    zone_type = getattr(resp, 'zone_type', '')
    ttl = str(getattr(resp, 'ttl', ''))
    serial = str(getattr(resp, 'serial', ''))
    status = getattr(resp, 'status', '')
    record_num = str(getattr(resp, 'record_num', ''))
    pool_id = getattr(resp, 'pool_id', '')
    project_id = getattr(resp, 'project_id', '')
    created_at = getattr(resp, 'created_at', '')
    updated_at = getattr(resp, 'updated_at', '')
    ep_id = getattr(resp, 'enterprise_project_id', '')
    proxy_pattern = getattr(resp, 'proxy_pattern', '')
    masters = getattr(resp, 'masters', []) or []
    routers = getattr(resp, 'routers', []) or []
    output += f"id: {zid}\nname: {name}\ndescription: {description}\nemail: {email}\nzone_type: {zone_type}\nttl: {ttl}\nserial: {serial}\nstatus: {status}\nrecord_num: {record_num}\npool_id: {pool_id}\nproject_id: {project_id}\nenterprise_project_id: {ep_id}\nproxy_pattern: {proxy_pattern}\ncreated_at: {created_at}\nupdated_at: {updated_at}\n"
    if masters:
        output += f"masters: {', '.join(masters)}\n"
    if routers:
        output += "routers:\n"
        for router in routers:
            router_id = getattr(router, 'router_id', '')
            router_region = getattr(router, 'router_region', '')
            router_status = getattr(router, 'status', '')
            output += f"  router_id={router_id}, router_region={router_region}, status={router_status}\n"
    print(output)


try:
    http_config = build_http_config()

    client = DnsClient.new_builder().with_http_config(http_config).with_credentials(
        BasicCredentials(AK, SK, args.project_id) if not SecurityToken else BasicCredentials(AK, SK, args.project_id).with_security_token(SecurityToken)).with_region(DnsRegion.value_of(Region)).build()
    if not client:
        print("无法获取 DNS 客户端")
        exit(-1)

    request = ShowPrivateZoneRequest()
    request.zone_id = args.zone_id

    response = client.show_private_zone(request)

    render(response)
except Exception as e:
    print(f"dns.show_private_zone 查询失败: {e}")
    exit(1)
