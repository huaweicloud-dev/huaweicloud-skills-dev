...
from huaweicloudsdkeip.v3.model import ListEipBandwidthsRequest
...
# 参数
parser = argparse.ArgumentParser(description="查询带宽列表（v3接口，返回更详细的带宽信息）")
...
parser.add_argument("--admin_state", type=str, help="带宽状态，如 normal/freezed")
...
def render(bandwidths, total_count=None, has_more=False, next_marker=None):
    ...
    output = f"id\tname\tsize\ttype\tbandwidth_type\tadmin_state\tpublicip_count\n"
    for bw in bandwidths:
        ...
        admin_state = getattr(bw, 'admin_state', '')
        publicip_info = getattr(bw, 'publicip_info', [])
        publicip_count = str(len(publicip_info)) if publicip_info else '0'
        output += f"{bw_id}\t{name}\t{size}\t{bw_type}\t{bandwidth_type}\t{admin_state}\t{publicip_count}\n"
...
try:
    ...
    print(f"eip.list_eip_bandwidths 查询失败: {e}")
    exit(1)
