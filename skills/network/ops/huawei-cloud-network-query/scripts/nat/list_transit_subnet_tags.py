...
from huaweicloudsdknat.v2.model import ListTransitSubnetTagsRequest
...
# 参数
parser = argparse.ArgumentParser(description="查询中转子网项目标签")
...
try:
    ...
    request = ListTransitSubnetTagsRequest()
    response = client.list_transit_subnet_tags(request)
    tags = response.tags

    if not tags:
        print(f"没有找到中转子网项目标签 (区域: {Region})")
        exit(0)
    ...
    print(f"nat.list_transit_subnet_tags 查询失败: {e}")
    exit(1)
