...
from huaweicloudsdknat.v2.model import ListPrivateNatsByTagsRequest, ListTagResourceInstancesRequestBody, Tags, Match
...
# 参数
parser = argparse.ArgumentParser(description="按标签过滤查询私网NAT网关实例")
...
def render(resources, total_count=None, has_more=False, next_marker=None):
    ...
    if total_count is not None:
        output += f"\n共 {total_count} 个匹配的私网NAT网关，当前返回 {showing_count} 个"
...
try:
    ...
    if not resources:
        print(f"没有找到匹配的私网NAT网关 (区域: {Region})")
        exit(0)
    ...
    print(f"nat.list_private_nats_by_tags 查询失败: {e}")
    exit(1)
