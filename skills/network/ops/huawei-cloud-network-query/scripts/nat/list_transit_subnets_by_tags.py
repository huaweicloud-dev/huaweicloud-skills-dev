...
from huaweicloudsdknat.v2.model import ListTransitSubnetsByTagsRequest, ListTagResourceInstancesRequestBody, Tags, Match
...
# 参数
parser = argparse.ArgumentParser(description="按标签过滤查询中转子网实例")
...
def render(resources, total_count=None, has_more=False, next_marker=None):
    ...
    if total_count is not None:
        output += f"\n共 {total_count} 个匹配的中转子网，当前返回 {showing_count} 个"
...
try:
    ...
    if args.action == "count":
        ...
        request = ListTransitSubnetsByTagsRequest()
        request.body = body
        response = client.list_transit_subnets_by_tags(request)
        total = getattr(response, 'total_count', 0)
        print(f"匹配的中转子网总数: {total}")
        exit(0)
    ...
    print(f"nat.list_transit_subnets_by_tags 查询失败: {e}")
    exit(1)
