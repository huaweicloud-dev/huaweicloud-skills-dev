...
from huaweicloudsdknat.v2.model import ShowTransitSubnetTagsRequest
...
# 参数
parser = argparse.ArgumentParser(description="查询指定中转子网的标签")
parser.add_argument("--project_id", type=str, default=None, help="项目 ID，不传时自动通过 IAM API 获取")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--resource_id", type=str, required=True, help="中转子网ID，可通过 list_transit_subnet.py 获取")
...
try:
    ...
    request = ShowTransitSubnetTagsRequest()
    request.resource_id = args.resource_id
    response = client.show_transit_subnet_tags(request)
    tags = response.tags
    ...
    print(f"nat.show_transit_subnet_tags 查询失败: {e}")
    exit(1)
