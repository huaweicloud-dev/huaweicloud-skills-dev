...
from huaweicloudsdknat.v2.model import ListNatGatewayTagRequest
...
# 参数
parser = argparse.ArgumentParser(description="查询公网NAT网关项目标签")
...
try:
    ...
    request = ListNatGatewayTagRequest()
    response = client.list_nat_gateway_tag(request)
    tags = response.tags

    if not tags:
        print(f"没有找到公网NAT网关项目标签 (区域: {Region})")
        exit(0)
    ...
    print(f"nat.list_nat_gateway_tag 查询失败: {e}")
    exit(1)
