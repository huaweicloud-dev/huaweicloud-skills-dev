...
from huaweicloudsdkelb.v3.model import ListLoadbalancerTagsRequest
...
try:
    ...
    request = ListLoadbalancerTagsRequest()
    response = client.list_loadbalancer_tags(request)
    items = response.tags
    if not items:
    ...
    print(f"elb.list_loadbalancer_tags 查询失败: {e}")
    exit(1)
