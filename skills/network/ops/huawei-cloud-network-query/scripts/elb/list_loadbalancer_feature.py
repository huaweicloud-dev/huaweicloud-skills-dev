...
from huaweicloudsdkelb.v3.model import ListLoadbalancerFeatureRequest
...
parser = argparse.ArgumentParser(description="查询 ELB 负载均衡器特性列表")
...
try:
    ...
    if args.marker:
        for i, item in enumerate(items):
            if str(getattr(item, 'feature', '')) == args.marker:
                start_idx = i + 1
                break
    ...
    next_marker = None
    if has_more:
        next_marker = str(getattr(remaining_items[PAGE_SIZE - 1], 'feature', ''))
    display_items = remaining_items[:PAGE_SIZE]

    output = f"feature\ttype\tvalue\n"
    ...
    print(f"elb.list_loadbalancer_feature 查询失败: {e}")
    exit(1)
