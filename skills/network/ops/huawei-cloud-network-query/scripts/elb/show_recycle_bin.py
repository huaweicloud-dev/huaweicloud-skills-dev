...
from huaweicloudsdkelb.v3.model import ShowRecycleBinRequest
from huaweicloudsdkelb.v3.region.elb_region import ElbRegion

AK, SK, Region, SecurityToken = load_credentials()

parser = argparse.ArgumentParser(description="查询 ELB 回收站配置")
...
try:
    ...
    item = response.recycle_bin
    if not item:
        print(f"没有找到回收站配置")
        exit(0)

    # 输出详情
    output = f"project_id\tenable\tpolicy\n"
    project_id = getattr(item, 'project_id', '')
    enable = getattr(item, 'enable', '')
    policy = getattr(item, 'policy', '')
    output += f"{project_id}\t{enable}\t{str(policy)}\n"
    print(output)
except Exception as e:
    print(f"elb.show_recycle_bin 查询失败: {e}")
    exit(1)
