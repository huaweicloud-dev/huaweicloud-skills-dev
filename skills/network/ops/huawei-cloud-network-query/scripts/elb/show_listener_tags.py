...
from huaweicloudsdkelb.v3.model import ShowListenerTagsRequest
from huaweicloudsdkelb.v3.region.elb_region import ElbRegion

AK, SK, Region, SecurityToken = load_credentials()
Offset = 0

parser = argparse.ArgumentParser(description="查询 ELB 监听器标签列表")
parser.add_argument("--project_id", type=str, default=None, help="项目 ID，不传时自动通过 IAM API 获取")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--listener_id", type=str, required=True, help="监听器 ID（必填），可通过 list_listeners.py 获取")
...
def render(items):
    total = len(items)
    if Offset >= total:
        print(f"查询结果为空\n\n监听器标签列表共 {total} 个, 序号从 0 开始, offset必须 < {total}")
        exit(0)
    output = f"key\tvalue\n"
    ...
    if total > 50 or Offset > 0:
        output += f"\n监听器标签列表共 {total} 个, 序号从 0 开始，当前显示第 {Offset}~{end} 个\n"
        if end + 1 < total:
...
try:
    ...
    request = ShowListenerTagsRequest()
    request.listener_id = args.listener_id
    response = client.show_listener_tags(request)
    items = response.tags
    if not items:
        print(f"没有找到监听器标签 (区域: {Region})")
        exit(0)
    render(items)
except Exception as e:
    print(f"elb.show_listener_tags 查询失败: {e}")
    exit(1)
