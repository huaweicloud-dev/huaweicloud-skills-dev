...
from huaweicloudsdkelb.v3.model import ListListenerTagsRequest
...
try:
    ...
    request = ListListenerTagsRequest()
    response = client.list_listener_tags(request)
    items = response.tags
    if not items:
    ...
    if args.marker:
        for i, item in enumerate(items):
            if str(getattr(item, 'key', '')) == args.marker:
                start_idx = i + 1
                break
    ...
    print(f"elb.list_listener_tags 查询失败: {e}")
    exit(1)
