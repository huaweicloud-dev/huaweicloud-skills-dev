...
from huaweicloudsdkelb.v3.model import ListFeatureConfigsRequest
...
parser = argparse.ArgumentParser(description="查询 ELB 特性配置列表")
parser.add_argument("--project_id", type=str, default=None, help="项目 ID，不传时自动通过 IAM API 获取")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--feature", type=str, required=True, help="特性名称，如 l7flow、elb_monitors 等")
...
try:
    ...
    output = f"id\tfeature\ttype\tvalue\tswitch\tdescription\tservice\ttenant_id\n"
    for item in display_items:
        id = getattr(item, 'id', '')
        feature = getattr(item, 'feature', '')
        type = getattr(item, 'type', '')
        value = getattr(item, 'value', '')
        switch = getattr(item, 'switch', '')
    ...
    print(f"elb.list_feature_configs 查询失败: {e}")
    exit(1)
