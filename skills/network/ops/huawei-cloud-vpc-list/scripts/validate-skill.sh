#!/usr/bin/env bash
# validate-skill.sh — Huawei Cloud Skill Specification validation for huawei-cloud-vpc-list
set -euo pipefail
...
check "Medium" "ListVpcs operation PascalCase" "$(grep -q 'ListVpcs' "$SKILL_MD" && echo PASS || echo FAIL)"
