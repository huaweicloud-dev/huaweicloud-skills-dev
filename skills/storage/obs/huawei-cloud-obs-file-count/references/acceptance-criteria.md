...
|---|-----------|--------------|
| AC-01 | Counts all files in a bucket | `python3 scripts/count_obs_files.py --bucket <bucket>` prints a single non-negative integer matching `hcloud OBS ls obs://<bucket>/ -limit=0` "File number: N" |
| AC-02 | Counts files under a prefix | `--prefix <prefix>` restricts the count to files under that prefix; `prefix` and `prefix/` forms return the same number |
| AC-03 | Folder markers excluded | Objects whose keys end with `/` (zero-byte folder markers) are not counted |
| AC-04 | Empty prefix returns 0 | A prefix with no objects returns `0` |
| AC-05 | Only the count is returned | Output is a single integer with no surrounding text |
| AC-06 | SDK fallback works | `--executor sdk` returns the same count as the CLI path |
...
| AC-Q2 | Script runs without syntax errors | `python3 scripts/count_obs_files.py --help` exits 0 |
