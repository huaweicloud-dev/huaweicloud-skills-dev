...
## Least-Privilege Policy for OBS File Counting

This skill is read-only and needs permission to list objects in a bucket so it can count files. The minimal IAM actions are:

- `obs:bucket:ListBucket` — list objects (with prefix/marker pagination) in the bucket
...
- Counting files under a prefix only needs `ListBucket`/`ListObjects` on the matching object path; a broader `object:*` resource is shown for simplicity and can be narrowed to `OBS:*:*:object:bucket-name/*`.
