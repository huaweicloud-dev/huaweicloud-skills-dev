...
check "Critical" "Mandatory quality SDK vendored" "$([ -f "$SKILL_PATH/scripts/skill_quality_sdk.py" ] && grep -q 'quality_report\|quality_context' "$SKILL_PATH/scripts/query_account_balance.py" && echo PASS || echo FAIL)"
check "Critical" "No credential hardcoding" "$(grep -RniE '(AK=|SK=|access.key=|secret.key=|password=)' "$SKILL_PATH" --include='*.md' --include='*.json' --include='*.sh' --include='*.py' --exclude='validate-skill.sh' | grep -viE 'your-ak|your-sk|your_access_key|your_secret_key|<ak>|<sk>|<your-[a-z-]+>|xxx' && echo FAIL || echo PASS)"
...
check "Medium" "SDK executor documented" "$(grep -q 'query_account_balance.py' "$SKILL_MD" && echo PASS || echo FAIL)"
