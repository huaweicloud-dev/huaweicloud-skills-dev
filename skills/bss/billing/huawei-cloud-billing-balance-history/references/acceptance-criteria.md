...
|---|-----------|--------------|
| 1 | Skill directory exists under `skills/bss/billing/huawei-cloud-billing-balance-history/` | `ls skills/bss/billing/huawei-cloud-billing-balance-history/` |
| 2 | `SKILL.md` exists with YAML frontmatter (`name`, `description`, `tags`, no `version`) | `bash scripts/validate-skill.sh skills/bss/billing/huawei-cloud-billing-balance-history` |
...
| 9 | Quality SDK vendored and integrated | `scripts/skill_quality_sdk.py` exists and `scripts/query_balance_history.py` uses `quality_context` |
| 10 | All three actions work with real credentials | `bash scripts/test-cli-commands.sh skills/bss/billing/huawei-cloud-billing-balance-history sdk` → PASS |
| 11 | JSON output is valid | `python3 scripts/query_balance_history.py --action balance --format json` parses with `jq`/`json.loads` |
