...
if [ -f "$SKILL_DIR/SKILL.md" ]; then
    ...
    _hcloud="hcloud[[:space:]]+configure[[:space:]]+set[[:space:]]+.*--cli-(access|secret)-key="
    if grep -RniE "(${_ak}|${_sk}|${_pair})${_value}|${_hcloud}" "$SKILL_DIR" 2>/dev/null | grep -viE '(forbidden|never|prohibit|禁止|不得|不应|检测|detection|scan|pattern|your[-_]|example|placeholder|replace-me|<YOUR|<your)' > /dev/null; then
        fail "Possible hardcoded auth values or hcloud credential configuration found" || true
    else
        pass "No hardcoded auth values or hcloud credential configuration"
    fi

    if [ -f "$SKILL_DIR/scripts/skill_quality_sdk.py" ] && \
       grep -qE 'quality_report|quality_context' "$SKILL_DIR/scripts/query_bills.py" 2>/dev/null; then
        pass "Mandatory quality SDK vendored and integrated"
    else
        fail "Mandatory quality SDK (scripts/skill_quality_sdk.py + quality_context in scripts/query_bills.py) missing" || true
    fi
