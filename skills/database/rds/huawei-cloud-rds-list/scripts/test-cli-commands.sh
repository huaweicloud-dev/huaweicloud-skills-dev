#!/usr/bin/env bash
# test-cli-commands.sh — Functional test for huawei-cloud-rds-list skill
set -uo pipefail
...
echo "=== RDS List Skill Test Script ==="
...
if [ "$EXECUTOR" = "cli" ]; then
  run_test "TC-01" "List All RDS Instances" \
    "hcloud RDS ListInstances --cli-region=$REGION --limit=10 --cli-output=json"

  run_test "TC-02" "Extract RDS Instance Names Only" \
    "hcloud RDS ListInstances --cli-region=$REGION --limit=10 --cli-query=\"instances[].name\""

  run_test "TC-03" "List RDS Instances Filtered By Engine" \
    "hcloud RDS ListInstances --cli-region=$REGION --datastore_type=MySQL --limit=10 --cli-output=json"

  run_test "TC-04" "List RDS Instances Filtered By Type" \
    "hcloud RDS ListInstances --cli-region=$REGION --type=Ha --limit=10 --cli-output=json"

  run_test "TC-05" "List RDS Instances With Pagination" \
    "hcloud RDS ListInstances --cli-region=$REGION --limit=5 --offset=0 --cli-output=json"

  run_test "TC-06" "Quality-Reporting Wrapper (names only, reporting disabled)" \
    "SKILL_QUALITY_DISABLE=1 python3 $SKILL_ROOT/scripts/list_rds_instances.py --region=$REGION --names-only --limit 10"
fi
