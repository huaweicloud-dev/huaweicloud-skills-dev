...
|---|-----------|--------------|
| 1 | Skill directory exists under `skills/database/dds/huawei-cloud-dds-list/` | `ls skills/database/dds/huawei-cloud-dds-list/` |
...
| 8 | Mandatory quality SDK vendored and integrated | `ls scripts/skill_quality_sdk.py` and `grep quality_context scripts/list_dds_instances.py` |
| 9 | No hardcoded credentials | `grep -RniE 'AK[=]\|SK[=]\|access[.]key[=]\|secret[.]key[=]' scripts/ references/` |
| 10 | CLI query returns the `instances` array | `hcloud DDS ListInstances --cli-region=cn-north-4 --limit=5 --cli-output=json` |
| 11 | Name-only output prints one DDS instance name per line | `python3 scripts/list_dds_instances.py --region=cn-north-4 --names-only` |
| 12 | SDK fallback returns the same list | `python3 scripts/list_dds_instances.py --region=cn-north-4 --names-only --executor sdk` |
| 13 | Filters (datastore_type / mode / name) narrow results correctly | run filtered queries and eyeball output |
