...
Configure AK/SK (an IAM user with at least `DDS ReadOnlyAccess`):
...
`403/401`, check the AK/SK and the IAM policy (`dds:instance:list` or
`DDS ReadOnlyAccess`).

## SDK fallback (optional)

The wrapper script `scripts/list_dds_instances.py` falls back to the Python
SDK when the CLI is unavailable:

```bash
pip install huaweicloudsdkcore huaweicloudsdkdds
```
