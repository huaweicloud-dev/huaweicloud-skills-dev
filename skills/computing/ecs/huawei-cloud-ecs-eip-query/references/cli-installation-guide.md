...
A successful run prints the EIP list in JSON format.
...
| `User has no permission` | IAM policy missing `vpc:publicIps:list` — see `references/eip-policies.md` |
| `Missing required parameter --cli-region` | Add `--cli-region=<region>` to the command |
| Empty result | The ECS has no bound EIP, or the region is wrong — verify with `hcloud ECS ShowServer` |
