...
check "Critical" "references/eip-policies.md exists" "$([ -f "$SKILL_PATH/references/eip-policies.md" ] && echo PASS || echo FAIL)"
...
check "Medium" "Service name EIP uppercase" "$(grep -q 'hcloud EIP' "$SKILL_MD" && echo PASS || echo FAIL)"
check "Medium" "Includes ListPublicips command" "$(grep -q 'hcloud EIP ListPublicips' "$SKILL_MD" && echo PASS || echo FAIL)"
