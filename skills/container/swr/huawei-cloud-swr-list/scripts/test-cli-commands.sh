...
echo "=== SWR Repository List Skill Test Script ==="
...
if [ "$EXECUTOR" = "cli" ]; then
  run_test "TC-01" "List All SWR Repositories" \
    "hcloud SWR ListReposDetails --cli-region=$REGION --limit=100"

  run_test "TC-02" "List Repositories With Pagination" \
    "hcloud SWR ListReposDetails --cli-region=$REGION --limit=10 --offset=0"

  run_test "TC-03" "List Repositories Sorted By Updated Time" \
    "hcloud SWR ListReposDetails --cli-region=$REGION --limit=100 --order_column=updated_time --order_type=desc"

  run_test "TC-04" "List Repositories With JSON Output" \
    "hcloud SWR ListReposDetails --cli-region=$REGION --limit=100 --cli-output=json"

  if [ -n "$NAMESPACE" ] && [ "$NAMESPACE" != "{namespace}" ]; then
    run_test "TC-05" "List Repositories In Namespace" \
      "hcloud SWR ListReposDetails --cli-region=$REGION --namespace=$NAMESPACE --limit=100"
  else
    echo "--- [TC-05] List Repositories In Namespace ---"
...
from huaweicloudsdkswr.v2.model import ListReposDetailsRequest
...
req = ListReposDetailsRequest()
req.limit = 100
resp = client.list_repos_details(req)
repos = resp.body if resp and resp.body else []
names = [r.name for r in repos]
print(f'PASS: ListReposDetails returned {len(repos)} repository(ies): {names}')
" 2>&1
  SDK_EXIT=$?
  set -e
